...
def track_particle_sixtrack(
    ...
    if mode == 'tbt':
        ...
        lines_f3[i_start_dp + 1] = 'ALL 1 664 101 dumtemp.dat\n'
    else:
    ...
    n_data = len(sixdump_all.particles)
    n_ele = int(n_data/(n_part*n_turns))

    x_tbt = np.zeros((n_turns*n_ele, n_part))
    px_tbt = np.zeros((n_turns*n_ele, n_part))
    y_tbt = np.zeros((n_turns*n_ele, n_part))
    py_tbt = np.zeros((n_turns*n_ele, n_part))
    sigma_tbt = np.zeros((n_turns*n_ele, n_part))
    delta_tbt = np.zeros((n_turns*n_ele, n_part))
